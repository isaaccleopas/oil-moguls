defmodule OilMogulsWeb.AcademyLive do
  use OilMogulsWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    {:ok, assign(socket, :page_title, "Academy")}
  end

  @impl true
  def render(assigns) do
    ~H"""
    <.page_hero kicker="Oil Moguls Academy" title="Master the skill the way working professionals do.">
    ...
    <section class="py-16 sm:py-24">
      <div class="om-container">
        <.section_heading kicker="Tracks" title="Curriculum with sector depth.">
          Named disciplines. No generic professional-development filler. Each track is designed to turn into work.
        </.section_heading>

        <div class="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <article :for={track <- tracks()} class="om-card p-6 sm:p-8">
    ...
    <section class="border-y border-line bg-white py-16 sm:py-24">
      <div class="om-container grid gap-12 lg:grid-cols-2">
        <.section_heading kicker="How we teach" title="Substance first. Credentials follow.">
          We respect expertise above titles. Members earn standing through demonstrated skill, not proximity to power.
        </.section_heading>
        <ul class="space-y-6">
          <li :for={item <- principles()} class="border-t border-line pt-6">
            <h3 class="font-semibold text-forest">{item.title}</h3>
            <p class="om-body mt-2 text-sm">{item.body}</p>
          </li>
        </ul>
      </div>
    </section>
      ...
      <div class="om-container flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div class="max-w-xl">
          ...
          <p class="om-body mt-3">
            Completing a module should lead somewhere: ApexBee practice, a mentor, a verified listing, a stronger profile.
          </p>
        </div>
        <div class="flex flex-col gap-3 sm:flex-row">
          <.link navigate="/join" class="om-btn om-btn-primary">Apply for the Academy</.link>
          <.link
            href={OilMoguls.apexbee_url()}
            target="_blank"
            rel="noopener noreferrer"
            class="om-btn om-btn-outline"
          >
            Practise on ApexBee
          </.link>
        </div>
  ...
  defp principles do
    [
      %{title: "Practice over slides", body: "Content has to produce a tangible outcome for a member's career. If it cannot, it does not ship."},
      %{title: "Verified instructors", body: "We do not platform unqualified experts. Mentors and faculty are vetted before they teach."},
      %{title: "Visible progress", body: "Completion is marked, shareable, and tied to the profile that recruiters and mentors actually read."}
    ]
  end
end
